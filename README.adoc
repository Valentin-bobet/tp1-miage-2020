...
#MESNAGE Matthieu# et #BOBET Valentin#
...
Le projet n'est pas structuré en couches. Dans le cadre d'un projet d'entreprise, cette structure pourrait devenir vite illisible et difficilement maintenable.

IMPORTANT: [Question]  Que se passe-t-il si l'on renomme une colonne de la base de donnée ?

Il faudra vérifier la concordance entre les attributs indiqués dans l'application et eventuellement mettre à jour le mapping manuellement pour éviter des erreurs. 
...
Le code métier se trouve dans le controlleur. Il n'est pas testable avec un test unitaire car la fonction utilise les attributs d'un autre objet. Cette fonction devrait être présente dans l'objet concerné.
...
docker run permet de lancer une image docker.
l'option --rm permet de supprimer le conteneur si celui-ci existe déjà
l'option -it permet d'écrire et recevoir les logs de la base directement sur le terminal
-e POSTGRES_PASSWORD=password permet de set la variable d'environnement password pour se connecter à la base.
-p permet de publier sur un certain port de l'hôte.
postgres nom de l'image.
...
La commande docker ps permet de voir le statut du conteneur docker, avec son id, l'image à partir de laquelle il est crée ainsi que la durée depuis laquelle le conteneur est crée. Il affiche aussi la durée pendant laquelle le conteneur à tourné.

La seconde commande indique toutes les actions effectuées sur le conteneur docker.
...
Le grand principe de cette architecture est de séparer la logique métier de la technique d'implémentation.
Cela permet de changer facilement de techno en modifiant les interfaces.
